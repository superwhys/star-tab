...
- 星球模式与书签搜索联动：匹配节点会发光高亮，星球自动旋转、放大并将当前结果定位到视图中央，结束搜索后恢复原视角。
...
- 可选使用 Chrome Storage Sync 在设备间同步背景、搜索引擎、布局和显示偏好。
...
本地开发预览使用浏览器 `localStorage` 的同名键作为替代。默认配置只保存在本机；在设置中开启“Chrome 配置同步”后，扩展会改用 `chrome.storage.sync`。书签本身仍由 Chrome 的书签同步功能管理。
...
Playwright 会自动启动本地 Vite 服务，并使用本机 Chrome 验证搜索、星图定位、书签弹层、设置保存、分组排序和动态背景。
