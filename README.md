...
- 展示当前时间、日期和可选秒数。
- 搜索框同时支持书签搜索和 Chrome 默认搜索引擎：
  - 输入内容时匹配整个 Chrome 书签树，包括深层文件夹中的书签。
  - 未选中书签时按回车，使用默认搜索引擎搜索。
...
- 默认搜索只显示“原型模式”提示，不会真正调用 Chrome 默认搜索引擎。
...
这是开发预览模式的设计。`chrome.search.query` 只能在扩展环境中使用，加载 `dist` 后会调用真实默认搜索引擎。
